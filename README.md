...
- `原模板`：只使用既有 `DAILY OPERATIONS REPORT` 解析规则，不启用兼容补齐，便于保持原解析行为和回归对照。
- `兼容模板（自动识别）`：在原解析结果上识别 `DAILY OPERATIONS SHORT REPORT` 等已登记变体；仅当身份字段缺失时，允许从符合命名规则的源文件名补齐日期、井号、日报号和井队。解析结果会在 `metadata` 中记录模板变体、兼容补齐字段和文件名补齐字段，便于审计。后续新增模板时应在独立的模板兼容模块中登记，不修改原模板解析器。
...
入库前会严格核对基本信息中的 `Event` 与上传入口的日报类型。钻井、完井、修井和搬迁分别使用独立解析策略并保存为独立 `report_type`；搬迁不会再借用钻井类型存储。钻井的原模板与兼容模板也分别注册，兼容规则不会回写原解析器。缺少事件、无法判断、类型冲突或入口不匹配时，整份 PDF（包括合并 PDF 中已识别的其他日期）均不会入库。
...
字段级翻译表不会复制整份日报 payload，也不会翻译井号、日期、时间、深度、单位和文件名等业务标识。页面点击 `原文 / 中` 时，后端按 `record_id + entity_type + entity_id + field_code + target_language` 读取 `translation_content`，校验 `source_hash` 后在响应阶段叠加译文，不改写原始字段或标准事实；西班牙语只用于菜单、标题、字段名、单位和提示等固定界面。
...
- `pdf_import_service.py`：四类 PDF 的独立策略注册、Event 类型门禁、身份校验和批量导入响应。
- `drilling_pdf_templates.py`：钻井原模板与兼容模板的独立选择层。
- `report_normalization_service.py`：把原始审计数据同步成标准日报、Operation、作业和质量事实。
- `operation_standardization.py`：Operation 工作分类、时效字段和描述规范化。
...
统计接口只读取 `vw_report_analytics` 和 `vw_rig_production_timeline` 等数据库视图，不建立月度统计快照表。原始 JSON 是可追溯层，`dpr_report`、`dpr_operation` 及各类型事实表是标准事实层，译文和 AI 提炼结果是按来源哈希关联的独立叠加层。
